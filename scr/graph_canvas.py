...
class GraphCanvas(ttk.Frame):
    ...
    def setup_axes(self):
        # Draw X-axis
        self.canvas.create_line(self.offset, self.canvas_height / 2 + self.offset,
                                self.canvas_width + self.offset, self.canvas_height / 2 + self.offset,
                                fill='white' if self.darkmode else 'black')  # X-axis

        # Draw Y-axis
        self.canvas.create_line(self.offset, self.offset,
                                self.offset, self.canvas_height + self.offset,
                                fill='white' if self.darkmode else 'black')  # Y-axis

        # Draw tick marks and labels for X-axis
        for i in range(0, 3):  # Changed to range from 0 to 2
            x = (self.canvas_width / 2 + (i - 1) *
                 self.canvas_width / 2) + self.offset
            self.canvas.create_line(x, (self.canvas_height / 2) - 10 + self.offset,
                                    x, (self.canvas_height / 2) + 10 + self.offset, fill='blue')
            self.canvas.create_text(
                x, (self.canvas_height / 2) + 10 + self.offset, text=f'{i}π' if i > 0 else '0', fill='white' if self.darkmode else 'black')

        # Draw tick marks and labels for Y-axis
        for i in range(-1, 2):
            y = (self.canvas_height / 2 - i *
                 (self.canvas_height / 2)) + self.offset
            self.canvas.create_line((self.offset) - 10, y,
                                    (self.offset) + 10, y, fill='red')  # Move Y-axis tick marks to the left
            self.canvas.create_text(
                (self.offset) + 20, y, text=f'{i}', fill='white' if self.darkmode else 'black')  # Adjust text position
    ...
    def draw_line(self, a, b, width=1, color='black'):
        ...
        new_r, new_g, new_b = utils.lighten_color(
            r // 256, g // 256, b // 256, 0.33)
    ...
    def _draw_extern_graph(self):
        ...
        for i, (name, (graph, color, width, graph_type, prio)) in enumerate(self.extern_graph.items()):
