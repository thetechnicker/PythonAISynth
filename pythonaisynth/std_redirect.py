...
from .utils import tk_after_errorless


class RedirectedOutputFrame(tk.Frame):
    def __init__(self, master=None, std_queue=None):
        super().__init__(master)
        self.textbox = scrolledtext.ScrolledText(self, height=10, font=("TkFixedFont"))
        self.textbox.configure(background=master.style.lookup("TFrame", "background"))
        self.textbox.configure(foreground=master.style.lookup("TFrame", "foreground"))
        ...
        self.ansi_font_format = {
            1: "bold",
            3: "italic",
            4: "underline",
            9: "overstrike",
        }
        self.ansi_font_reset = {
            21: "bold",
            23: "italic",
            24: "underline",
            29: "overstrike",
        ...
        self.textbox.tag_configure("foreground default", foreground=self.textbox["fg"])
        self.textbox.tag_configure("background default", background=self.textbox["bg"])
        ...
        for i, (col_dark, col_light) in enumerate(
            ...
            # tag configuration
            self.textbox.tag_configure("foreground " + col_dark, foreground=col_dark)
            self.textbox.tag_configure("background " + col_dark, background=col_dark)
            self.textbox.tag_configure("foreground " + col_light, foreground=col_light)
            self.textbox.tag_configure("background " + col_light, background=col_light)
    ...
    def insert_ansi(self, txt, index="insert"):
        first_line, first_char = map(int, str(self.textbox.index(index)).split("."))
        if index == "end":
            first_line -= 1
    ...
    def check_queue(self):
        ...
        # delay = 100
        delay = int(
            500 if self.queue.empty() else min(5, 100 / min(1, self.queue.qsize() / 10))
        )
